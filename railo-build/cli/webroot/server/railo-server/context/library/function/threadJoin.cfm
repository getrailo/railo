<cffunction name="threadJoin" output="no" returntype="void" hint="Makes the current thread wait until the thread or threads specified in the name attribute complete processing, 
or until the period specified in the timeout argument passes, before continuing processing. If you don't specify a timeout and thread you are joining
	...
	<cfargument name="name" type="string" required="yes" hint="The name of the thread or threads to join to the current thread. To specify multiple threads, use a comma-delimited list.">
	<cfargument name="timeout" type="numeric" required="no" default="#0#" hint="The number of milliseconds that the current thread waits for the thread or threads being joined to finish. 
