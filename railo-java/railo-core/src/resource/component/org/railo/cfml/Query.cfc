component output="false" extends="Base" accessors="true"{
	...
	private Array function parseSql(){
		...
		while ( Pos LTE TotalLen )
		{
			...
			{
				var Len = 1 ;
				while ( Mid(Sql,Pos+Len,1) NEQ NextChar AND Pos LTE TotalLen  &&(( Pos + Len ) LTE TotalLen ) )
				{
					Len++ ;
			...
			if (Pos EQ StartPos)
			{
				...
				else
				{
					...
					throw(type="org.railo.cfml.query.SqlUnknownSyntaxError", message="The sql statement [#sql#] contains an unknown syntax error.");
