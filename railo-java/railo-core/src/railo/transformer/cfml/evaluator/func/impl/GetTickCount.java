	...
	public void evaluate(BIF bif, FunctionLibFunction flf) throws TemplateException {
		...
		if(value instanceof LitString) {
			...
			if("nano".equalsIgnoreCase(unit))
				arg.setValue(bif.getFactory().createLitDouble(railo.runtime.functions.other.GetTickCount.UNIT_NANO), "number");
			else if("milli".equalsIgnoreCase(unit))
				arg.setValue(bif.getFactory().createLitDouble(railo.runtime.functions.other.GetTickCount.UNIT_MILLI), "number");
			else if("micro".equalsIgnoreCase(unit))
				arg.setValue(bif.getFactory().createLitDouble(railo.runtime.functions.other.GetTickCount.UNIT_MICRO), "number");
			else if("second".equalsIgnoreCase(unit))
				arg.setValue(bif.getFactory().createLitDouble(railo.runtime.functions.other.GetTickCount.UNIT_SECOND), "number");
		}
	}
