...
public class CFMLOrJavaScriptTransformer implements TagDependentBodyTransformer {
	...
	@Override
	public void transform(Factory factory,Page page,CFMLTransformer parent, EvaluatorPool ep,FunctionLib[] flibs, Tag tag, TagLibTag tagLibTag,TagLibTag[] scriptTags, CFMLString cfml,TransfomerSettings settings) 
	throws TemplateException {
		...
		if(attr!=null) {
			...
			if("java".equalsIgnoreCase(str))		jst.transform(factory,page,parent, ep, flibs, tag, tagLibTag,scriptTags, cfml,settings);
			else if("cfml".equalsIgnoreCase(str))	cst.transform(factory,page,parent, ep, flibs, tag, tagLibTag,scriptTags, cfml,settings);
			else 
