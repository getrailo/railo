...
public final class ClientCookieOld extends ClientSupport {
	private static final long serialVersionUID = 5480191870824730047L;
	 ...
	 * @param sct
	 */
	private ClientCookieOld(PageContext pc,String cookieName,Struct sct) {
		super(
				sct,
	 ...
	 * @param other
	 */
	private ClientCookieOld(ClientCookieOld other,boolean deepCopy) {
	...
	public static Client getInstance(String name, PageContext pc) {
		...
		if(data!=null) {			
			try {
				...
				return new ClientCookieOld(pc,cookieName,sct);
			} 
			catch (Exception e) {
				
			}
		}
		return new ClientCookieOld(pc,cookieName, new StructImpl());
	}
	...
	public Collection duplicate(boolean deepCopy) {
    	return new ClientCookieOld(this,deepCopy);
	}
	
	private static Struct decode(PageContext pc, String data) throws PageException {
		return (Struct) evaluator.interpret(pc,data);
	}
}
