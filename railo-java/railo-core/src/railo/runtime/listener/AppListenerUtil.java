...
public final class AppListenerUtil {
	...
	public static PageSource getApplicationPageSourceRoot(PageContext pc, RefBoolean isCFC) {
		PageSource ps = ((PageContextImpl)pc).getPageSourceExisting("/Application.cfc");
		if(ps!=null) {
			isCFC.setValue(true);
	    	return ps;
		}
		ps = ((PageContextImpl)pc).getPageSourceExisting("/Application.cfm");
		if(ps!=null) return ps;
		return null;
	}
	...
	public static PageSource getApplicationPageSourceCurr2Root(PageContext pc,PageSource requestedPage, RefBoolean isCFC) {
		...
		for(int i=arr.size()-1;i>0;i--) {
			...
			path=sb.toString();
			res = ((PageContextImpl)pc).getPageSourceExisting(path.concat("Application.cfc"));
			if(res!=null) {
				isCFC.setValue(true);
				return res;
			}

			res = ((PageContextImpl)pc).getPageSourceExisting(path.concat("Application.cfm"));
	...
	public static void setORMConfiguration(PageContext pc, ApplicationContextPro ac,Struct sct) throws PageException {
		...
		ac.setORMConfiguration(ORMConfiguration.load(config,ac,sct,res,config.getORMConfig()));
