...
private static void main_adj(int repeat,String name,double[] arr, String list)
{
  ...
  //System.out.println();
    ...
    public void append(StringBuffer s, double d) {
        ...
        if (d == Double.NEGATIVE_INFINITY) s.append(NEGATIVE_INFINITY);
        else if (d == Double.POSITIVE_INFINITY) s.append(POSITIVE_INFINITY);
        else if (d != d) s.append(NaN);
        else if (d == 0.0) {
            if ( (Double.doubleToLongBits(d) & DoubleSignMask) != 0)
                s.append('-');
