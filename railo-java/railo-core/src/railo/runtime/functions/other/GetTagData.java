...
public final class GetTagData implements Function {
	...
	private static Struct cfmlBasedTag(PageContext pc, TagLib tld, TagLibTag tag) throws PageException {
		...
		Component cfc = ComponentLoader.loadComponent(pc,source.getPageSource(), source.getFilename().substring(0,source.getFilename().length()-(pc.getConfig().getComponentExtension().length()+1)), false,true);
