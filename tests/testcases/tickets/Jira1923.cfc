...
component extends="org.railo.cfml.test.RailoTestCase"	{
	...
	public void function testForm(){
		...
		assertEquals("form:'1,2';",trim(result.filecontent));
	...
	public void function testURL(){
		// FALSE
		http method="get" result="local.result" url="#createURL("Jira1923/index.cfm?sameFormFieldsAsArray=false")#" addtoken="false" {
			httpparam type="url" name="test" value="1";
			httpparam type="url" name="test" value="2";
		}
		assertEquals("url:'1,2';",trim(result.filecontent));
		
		// TRUE
		http method="get" result="local.result" url="#createURL("Jira1923/index.cfm?sameFormFieldsAsArray=true")#" addtoken="false" {
			httpparam type="url" name="test" value="1";
			httpparam type="url" name="test" value="2";
		}
		assertEquals("url:[1,2];",trim(result.filecontent));
